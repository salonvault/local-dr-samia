...
export default function StepsSection() {
  return (
    <section id="process" className="relative overflow-visible bg-[var(--color-primary-3)] py-16 text-[var(--color-foreground)] lg:py-24">
      <div className="absolute inset-0 opacity-60 [background-image:linear-gradient(rgba(156,49,70,0.055)_1px,transparent_1px),linear-gradient(90deg,rgba(156,49,70,0.04)_1px,transparent_1px)] [background-size:42px_42px]" />
        ...
        <div className="grid gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
          <div className="grid gap-5 lg:min-h-[180vh]">
            {steps.map((step, index) => (
              <article
                key={step.number}
                className="sticky rounded-[8px] border border-[rgba(156,49,70,0.14)] bg-white/88 p-6 shadow-[var(--shadow-card)] backdrop-blur lg:min-h-[430px] lg:p-8"
              ...
              >
                <div className="flex items-start justify-between gap-6">
                  ...
                  <span className="hidden font-heading text-[68px] font-semibold leading-none text-[rgba(223,89,107,0.14)] sm:block">
                    {step.number}
                  </span>
                </div>
                ...
                <ul className="mt-8 grid gap-2">
                  {step.bullets.map((bullet) => (
                    <li key={bullet} className="flex items-start gap-3 rounded-[8px] border border-[rgba(156,49,70,0.12)] bg-[var(--warm-cream)]/72 p-4 text-[13px] leading-[1.65] text-[var(--color-ink-2)]">
          ...
          <div className="lg:sticky lg:top-24">
            <div className="relative min-h-[420px] overflow-hidden rounded-[8px] border border-[rgba(156,49,70,0.14)] bg-white shadow-[var(--shadow-lift)] lg:h-[min(620px,calc(100vh-7rem))] lg:min-h-0">
              <Image
                src="/assets/new/happy.jpeg"
                quality={100}
                alt="Patient receiving a professional skin treatment at Dr Samia Skin Clinic"
                fill
              ...
              <div className="absolute bottom-6 left-6 right-6 text-white">
                <p className="text-[10px] font-extrabold uppercase tracking-[0.08em] text-white/78">The Dr Samia care journey</p>
